...
dataOrParameterUI <- function(id, 
                              lsPreset = NULL, 
                              hL = 4, 
                              colWidth = 12, 
                              inpWidth = DEFAULT_INP_WIDTH,
                              paramType = c("fixed", "monitored")[2]) {
  
  if (length(intersect(paramType, c("fixed", "monitored"))) != 1) {
    stop(sprintf("paramType ('%s') must be either 'fixed' or 'monitored'.", paramType))
  }
  ...
  if (paramType == "fixed") {
    ...
    tl[[length(tl) +1]] <- GtextInput(
      id = ns('values_applied'),
      title = 'Value',
  ...
  tl[[length(tl) +1]] <- GtextAreaInput(
    id = ns('methods_and_procedures'),
  ...
  if (paramType == "monitored") {
    
    tl[[length(tl) + 1]] <- GtextAreaInput(
      id = ns('monitoring_frequency'),
...
dataOrParameterServer <- function(id,  
                                  lsPreset = NULL,
                                  paramType = c("fixed", "monitored")[2]) {
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      rvToReturn$results <- reactiveValues(
        ...
        comment = NULL)
      if (paramType == "fixed") {
        rvToReturn$results$values_applied <- NULL
      } else {
      ...
      if (paramType == "fixed" & ('values_applied' %in% names(lsPreset))) {
        updateTextInput(session = session, 

...
# Generic document review module.

genericDocumentReviewSchemaUI <- function(
    id, 
    lsPreset = NULL, 
    hL = 4, 
    colWidth = 12, 
  ...
  tagList(
    ...
    chooseLicenseInput(
      ...
      helpTxt = "Select your Independent Impact license to use for this review."),
...
#'  
genericDocumentReviewSchemaServer <- function(
    id,  
    dbCon,
    loginInfoUsr,
    idEntity,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      modSrvrs$mChooseLic <- chooseLicenseServer(
        id = "chLicRev",
        dbCon = dbCon,
        scopes = licenseReq, 
        statuses = "ACTIVE",
      ...
      {
        schemaIri <- dbGetQuery(
          conn = dbCon, 
          statement = sprintf(
            "SELECT iri FROM tbl_schemas WHERE id = '%s';", idSchemaOrig))
        sch <- jsonlite::read_json(
          path = sprintf("%s%s.json", schemadir, schemaIri),
          simplifyVector = TRUE,
        ...
        idx <- which(dfSchema$key == "headers")
        if (length(idx) > 0) {
          dfSchema <- dfSchema[-idx,]
        }
      }
      ...
      observe({
        ...
        rvToReturn$results$headers$id_msg_lic <- modSrvrs$mChooseLic$results$id_msg
        rvToReturn$results$headers$url_ipfs_lic <- modSrvrs$mChooseLic$results$url_ipfs
        
        if (modSrvrs$mChooseLic$allResultsGood) {

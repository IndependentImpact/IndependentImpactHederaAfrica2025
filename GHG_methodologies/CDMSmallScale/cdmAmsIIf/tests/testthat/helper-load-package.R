# Ensure cdmAmsIIf functions are available when tests are run outside
...
if (!"cdmAmsIIf" %in% loadedNamespaces()) {
  find_pkg_root <- function(path = getwd()) {
    ...
    repeat {
      ...
      if (file.exists(desc_path)) {
        ...
        if (length(pkg_line) && grepl("\\bcdmAmsIIf\\b", pkg_line[1])) {
          return(path)
        }
      }
      parent <- dirname(path)
      if (identical(parent, path)) {
        stop("Package root for cdmAmsIIf not found.", call. = FALSE)
      }
      path <- parent
    }
  }
